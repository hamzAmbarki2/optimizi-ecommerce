...
const Orders: React.FC = () => {
  ...
  const getOrderStats = () => {
    ...
    // Provide legacy-friendly keys used in the UI
    const processing = preparing + confirmed; // treat confirmed+preparing as processing
    const shipped = outForDelivery; // shipped maps to out_for_delivery

    return { total, pending, confirmed, preparing, outForDelivery, delivered, processing, shipped };
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-gray-600">{t('orders.manage_description')}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-gray-600">{t('orders.total_orders')}</p>
      ...
      <div className="bg-white rounded-3xl p-6 shadow-lg">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">{t('orders.search_orders')}</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">{t('orders.status_filter')}</label>
            <select
              value={statusFilter}
            ...
            >
              ...
              <option value="processing">{t('orders.preparing')}</option>
              <option value="shipped">{t('orders.delivered')}</option>
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {filteredOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">TND{order.total?.toFixed(2)}</div>
                      <div className="text-xs text-gray-500">
                        Subtotal: TND{order.subtotal?.toFixed(2)}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">

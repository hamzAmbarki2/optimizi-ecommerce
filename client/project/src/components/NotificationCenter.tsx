...
export default function NotificationCenter() {
  ...
  // Local storage keys scoped by user id
  ...
  const lastStatusesKey = `${storagePrefix}:lastOrderStatuses`;
  ...
  if (!orders.length) return;
    ...
    orders.forEach(order => {
      ...
      // Skip if we already have a notification for this order/status
      const alreadyNotified = notifications.some(n => n.orderId === order.id && n.status === order.status);

      // If this is a new order or status has changed and we haven't already notified
      if ((!lastStatus || lastStatus !== order.status) && !alreadyNotified) {
    ...
    // Update the ref
    lastOrderStatusesRef.current = statusMap;
  }, [orders]);

  // Load persisted notifications and last statuses for the current user
  useEffect(() => {
    if (!state.isAuthenticated || !state.user) return;

    try {
      const storedNotifs = localStorage.getItem(notificationsKey);
      if (storedNotifs) {
        const parsed: Notification[] = JSON.parse(storedNotifs);
        // revive Date objects and recreate icons
        parsed.forEach(n => {
          if (typeof n.timestamp === 'string') n.timestamp = new Date(n.timestamp);
          n.icon = getNotificationIcon(n.type); // Recreate icon from type
        });
        setNotifications(parsed);
      }

      const storedStatuses = localStorage.getItem(lastStatusesKey);
      if (storedStatuses) {
  ...
  useEffect(() => {
    if (!state.isAuthenticated || !state.user) return;
    try {
      // Convert Date to ISO for storage, exclude icon (not serializable)
      const serializable = notifications.map(n => ({
        ...n,
        timestamp: n.timestamp instanceof Date ? n.timestamp.toISOString() : n.timestamp,
        icon: undefined // Exclude icon from storage
      }));
      localStorage.setItem(notificationsKey, JSON.stringify(serializable));
    } catch (err) {
      console.warn('Failed to persist notifications:', err);
    }
  }, [notifications, state.isAuthenticated, state.user]);

  // Persist last order statuses
  useEffect(() => {
    if (!state.isAuthenticated || !state.user) return;
    try {
      localStorage.setItem(lastStatusesKey, JSON.stringify(lastOrderStatusesRef.current));
    } catch (err) {
      console.warn('Failed to persist last order statuses:', err);
    }
  }, [state.isAuthenticated, state.user]);
  ...
  const getNotificationIcon = (type: Notification['type']) => {
    const icons = {
      ...
      secondary: <Package className="w-5 h-5" />,
      primary: <Truck className="w-5 h-5" />,

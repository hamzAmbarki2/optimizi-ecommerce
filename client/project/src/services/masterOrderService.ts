...
export const masterOrderService = {
      ...
      for (const [fournisseurId, fournisseurItems] of itemsByFournisseur) {
        ...
        try {
          await addDoc(collection(db, 'notifications'), {
            ...
            message: `Nouvelle commande de ${orderData.userName} - Total: TND${fournisseurTotal.toFixed(2)}`,
  ...
  async syncMasterOrderStatus(masterOrderId: string): Promise<void> {
    try {
      ...
      // After updating master order, notify the customer via email about the status/payment changes
      try {
        const masterSnap = await getDoc(masterOrderRef);
        if (masterSnap.exists()) {
          const updatedMaster: any = { id: masterSnap.id, ...(masterSnap.data() as any) };
          try {
            await emailService.sendOrderStatusUpdate(updatedMaster);
            console.log('✉️ [Client] Sent order status update email to customer');
          } catch (emailErr) {
            console.error('❌ [Client] Failed to send order status update email:', emailErr);
          }
        }
      } catch (err) {
        console.error('❌ [Client] Error fetching master order after update for email notification:', err);
      }
    } catch (error) {
